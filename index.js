//EXEMPLO1: PARA SABER SE É POSITIVO OU NEGATIVO OU 0

// let numero = prompt('Digite um número');


// if (numero > 0) {
//     document.write('o número é positivo');
// } else if (numero < 0) document.write('o número é negativo');
// else {
//     document.write('o número é 0');
// }

//EXEMPLO2: COMPRIMENTO DE UM TRIANGULO

// const ladoA = prompt('Digite o comprimento do lado A: ');
// const ladoB = prompt('Digite o comprimento do lado B: ');
// const ladoC = prompt('Digite o comprimento do lado C: ');

// if(ladoA ===  ladoB && ladoB === ladoC){
// document.write('o triângulo é equilátero');
// } else if (ladoA != ladoB && ladoB != ladoC) {
//     document.write('é um triângulo escaleno');
// } else {
//     document.write('é um triângulo isósceles');
// }

//EXEMPLO3: Verificar se um ano é bissexto. (Um ano é bissexto se for divisível por 4.
//Não é bissexto se for divisível por 100, a menos que também seja divisível por 400.)

// function bissexto(ano) {
//     return (ano % 4 === 0 && ano % 100 !== 0) || (ano % 400 === 0);
// }
// let ano = prompt('Digite um ano');

// if (bissexto (ano)){
// document.write('o ano ' + ano + ' é bissexto.')
// } else {
//     document.write('o ano ' + ano + ' não é bissexto')
// }

//EXEMPLO4: Determinar o maior entre três números. (dificil)

// let numero1 = prompt('Digite um número');
// let numero2 = prompt('Digite outro número');
// let numero3 = prompt('Digite mais um número');

// if (numero1 > numero2 && numero3){
//     document.write('o ' + numero1 + ' é o maior');

// }else if(numero2 > numero1 && numero3){
//     document.write('o ' + numero2 + ' é o maior');

// }else{
//     document.write('o ' + numero3 + ' é o maior');
// }

//EXEMPLO5: Verificar se um número é par ou ímpar.

// let numero = prompt('Digite um número');

// if (numero % 2 == 0) {
//     document.write('É par');
// } else {
//     document.write('É Ímpar')
// }

//EXEMPLO6: Calcular o preço final de um produto com base na quantidade comprada. (DIFÍCIL)

//EXEMPLO7: Converter uma nota em conceito. (DIFÍCIL)

// let nota = prompt("Digite uma nota de 0 a 100:")
// function determinarConceito(nota) {
//     let conceito
// };
// if (nota >= 80 && nota <= 100) {
//     conceito = 'A';
//     document.write('A')
// } else if (nota >= 60 && nota < 80) {
//     conceito = 'B';
//     document.write('B')
// } else if (nota >= 40 && nota < 60) {
//     conceito = 'C';
//     document.write('C')
// } else if (nota >= 20 && nota < 40) {
//     conceito = 'D';
//     document.write('D')
// } else if (nota >= 0 && nota < 20) {
//     conceito = 'F';
//     document.write('F')
// }

//EXEMPLO8: Verificar se uma letra é vogal ou consoante. (DIFICIL)

// let letra = prompt("Digite uma letra:");
// if (letra = 'a' || 'e' ||'i'|| 'o' ||'u') {
//     document.write('A letra ' + letra + ' é uma vogal.');
// } else {
//     document.write('A letra ' + letra + ' é uma consoante.');
// }

//EXEMPLO9: Determinar o dia da semana com base em um número. (DIFICIL)

//EXEMPLO10: Calcular o imposto de renda de uma pessoa com base em sua renda anual. (DIFÍCIL)






